...
#View(merged_counts)
>>>>>>> 11046d0c5eb02a9e8e794c6a484bcd43ccea8ac6
...
merged_counts_combined = merged_all <- merge(x= merged_counts,y= counts1,by="Gene.ID" ,all.x=TRUE)
merged_metadata_combined <- rbind(merged_metadata, metadata1)
# creating combined deseq object

merged_counts <- na.omit(merged_counts_combined) 
dds_combined <- DESeqDataSetFromMatrix(countData=merged_counts_combined , colData=merged_metadata_combined, design= ~compound,tidy=TRUE)
dds_combined <- DESeq(dds_combined)
```
...
```{r}
View(genes.symbols1)
#merged_genes <- merge(x= genes.symbols2,y= genes.symbols3) # R is stuck here dont run
``` 
...
#merged_genes1 <- merge(x= genes.symbols1,y= genes.symbols2)
#merged_genes2 <- merge(x= merged_genes1,y= genes.symbols3)

resultsNames(dds_combined)
resLFC_combined <- lfcShrink(dds_combined, contrast = c("compound", "ZNA", "DMSO","E2") , type="ashr")
resLFC_combined$symbol <- c(genes.symbols1, genes.symbols2, genes.symbols3)
resOrdered_combined <- resLFC_combined[order(resLFC_combined$pvalue),]

# Take top 10 genes with the lowest p-value that express in E2-treatment patients (log2FoldChange>0)
selectUp <- resOrdered1$symbol[resOrdered1$log2FoldChange>0][1:10]
# Take top 10 genes with the lowest  p-value that express in ZNA-treatment patients (log2FoldChange<0)
selectDown <- resOrdered1$symbol[resOrdered1$log2FoldChange<0][1:10]
select = c(selectUp,selectDown)
