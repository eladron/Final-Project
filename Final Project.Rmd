...
The count matrix of ZNA samples need to be divided into 2 groups, one for 3 hrs of treatment and the other for 12 hrs of treatment
```{r}
...
```{r}
metadata3 <- data.frame(metadata2[c(2,3,4,6,7,9),]) # 12 hrs
...
we will use deseq anylsis for each of the datasets.
```{r}
...
We will do volcano plot for each count matrix,
we will use p-Value cut off of 103-10. 
```{r}
# Dataset1
...
We run PCA between the DMSO samples and the E2 samples to find how significant was the effect of the treatment.
...
We run PCA between the DMSO samples,E2 samples and ZNA samples after 3 hours to determine how differnet the treatments are.
...
We run PCA between the ZNA samples to see how the duration of the treatment affects.
...
We did enrichment anylsis between the duration of ZNA treatment to see which pathways were expressed
...
######################################################
##                                                  ##
##      Restart the data for further analysis       ##
##                                                  ##
######################################################

almost identical to the start, it is needed for the rest of the anaylsis.
...
merged_counts_combined[apply(merged_counts_combined[,-1], 1, function(x) !all(x==0)),] # remove zeros
...
rownames(dds.symbol) <- mapIds(org.Hs.eg.db,
                              keys=rownames(dds_combined),
                              column="SYMBOL",
                              keytype="ENSEMBL",
                              multiVals="first")
rownames(dds.symbol)[is.na(rownames(dds.symbol))] <- rownames(dds_combined)[is.na(rownames(dds.symbol))]
rownames(dds.symbol) <- make.unique(rownames(dds.symbol))
```
...
df <- data.frame(row.names = colnames(dds.symbol),
                 compound = colData(dds.symbol)$compound)
# Get normalized counts
normcounts = assay(vst(dds.symbol, blind=T))
pheatmap(normcounts[select,], cluster_rows=TRUE,
         show_colnames = FALSE, cluster_cols=TRUE, 
         annotation_col=df, scale = 'row', cutree_cols = 3, cutree_rows = 2)
```
